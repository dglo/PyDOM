...
class Driver(object):
    ...
    def __init__(self, root='/proc/driver/domhub'):
        self.root = root
        f = open(os.path.join(self.root, "revision"))
        revstr = f.read()
    ...
    def scan(self):
        ...
        cards = list(filter(cre.match, os.listdir(self.root)))
        for c in cards:
            pairs = list(filter(pre.match, os.listdir(os.path.join(self.root, c))))
            ci = CardInfo(int(cre.match(c).group(1)))
            self.cards.append(ci)
            f = open(os.path.join(self.path(ci.id), "fpga"))
            while True:
                s = f.readline()
                if s == "":
                    break
            ...
            for p in pairs:
                ...
                f = open(os.path.join(self.path(ci, pi), "is-plugged"))
                try:
                    f.read().index("not")
                    pi.plugged = 0
                except:
                ...
                f = open(os.path.join(self.path(ci, pi), "pwr"))
                try:
                    f.read().index("off")
                    pi.powered = 0
                except:
    ...
    def on(self, card, pair):
        ...
        f = open(os.path.join(self.path(card, pair), "pwr"), "w")        
        f.write("on\n")
        f.close()
        return self.scan()
    ...
    def onAll(self):
        ...
        f = open(os.path.join(self.root, "pwrall"), "w")
        f.write("on\n")
        f.close()
        return self.scan()
    ...
    def softboot(self, domId):
        ...
        f = open(os.path.join(self.path(card, pair, dom), "softboot"))
        f.write("reset\n")
        f.close()
        return self.scan()
    
    def get_dom_id(self, card, pair, dom):
        f = open(os.path.join(self.path(card, pair, dom), "id"))
        s = f.read()
        # print "get_dom_id(): ", s
    ...
    def get_current(self, card, pair):
        f = open(os.path.join(self.path(card, pair), "current"))
    ...
    def print_dom_table(self, out=sys.stdout):
        ...
        print("CARD PAIR DOM Driver-File         DOMId", file=out)
        for (domid, loc) in list(self.doms.items()):
            print(" %2.2d   %2.2d   %s /dev/dhc%dw%dd%s %s" % (
        ...
        for c in self.cards:
            for p in c.pairs:
                if p.plugged == 1 and p.powered == 1:
                    for d in ('A', 'B'):
                        f = open(os.path.join(self.path(c, p, d),
                                              "is-communicating"), "r")
                        try:
                            f.read().index("not")
                        except ValueError:
    ...
    def enable_blocking(self, state):
        ...
        f = open(os.path.join(self.root, 'blocking'), 'w')
        f.write(str(state) + "\n")
        f.close()
...
class Power(object):
    ...
    def __init__(self, card, pair):
        ...
        self.fpwr = open(self.ptop + "/pwr", "w")
        self.card = card
        self.pair = pair
    ...
    def on(self):
        ...
        stat = [ ]
        for dom in ('A', 'B'):
            fcq = open(self.ptop + "/dom" + dom + "/is-communicating", "r")
            s = fcq.readline()
            fcq.close()
            try:
                s.index("NOT")
